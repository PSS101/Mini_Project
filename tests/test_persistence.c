...
static void test_aof_rewrite(void) {
    TEST_SUITE("Persistence / AOF Rewrite (Compaction)");

    remove(TEST_AOF_FILE);

    /* Create store, populate, rewrite, then reload */
    Store *db1 = init_engine(64);

    /* Simulate many writes that would normally bloat the AOF */
    char c1[] = "SET counter 0";   run_command(db1, c1);
    char c2[] = "INCR counter";    run_command(db1, c2);
    char c3[] = "INCR counter";    run_command(db1, c3);
    char c4[] = "INCR counter";    run_command(db1, c4);
    char c5[] = "SET name alice";  run_command(db1, c5);
    char c6[] = "SET name bob";    run_command(db1, c6);  /* overwrites alice */
    char c7[] = "RPUSH mylist x";  run_command(db1, c7);
    char c8[] = "RPUSH mylist y";  run_command(db1, c8);
    char c9[] = "SADD myset a";   run_command(db1, c9);
    char c10[] = "SADD myset b";  run_command(db1, c10);

    /* Rewrite: should produce a minimal AOF */
    int res = aof_rewrite(db1, TEST_AOF_FILE);
    ASSERT_EQ_INT(res, 0);
    aof_close();
    destroy_engine(db1);
    ...
    /* Verify data survived rewrite */
    char g1[] = "GET counter";
    CommandResult r = run_command(db2, g1);
    ASSERT_EQ_STR(r.message, "3");
    freeResult(r);

    char g2[] = "GET name";
    r = run_command(db2, g2);
    ASSERT_EQ_STR(r.message, "bob");
    freeResult(r);

    char g3[] = "LLEN mylist";
    r = run_command(db2, g3);
    ASSERT_EQ_STR(r.message, "2");
    freeResult(r);

    char g4[] = "SISMEMBER myset a";
    r = run_command(db2, g4);
    ASSERT_EQ_STR(r.message, "1");
    freeResult(r);
