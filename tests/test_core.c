...
#include "../src/pubsub/pubsub.h"

static void test_cluster_hashslot(void) {
    TEST_SUITE("Core / Cluster Hash Slots");
    ClusterState cs;
    cluster_init(&cs, "127.0.0.1", 6379);

    /* Slot should be deterministic and in range */
    unsigned int s1 = cluster_keyslot("foo");
    unsigned int s2 = cluster_keyslot("bar");
    unsigned int s3 = cluster_keyslot("foo");  /* same key = same slot */
    ASSERT_TRUE(s1 < CLUSTER_SLOTS);
    ASSERT_TRUE(s2 < CLUSTER_SLOTS);
    ASSERT_EQ_INT((int)s1, (int)s3);  /* deterministic */
    ASSERT_TRUE(s1 != s2);            /* different keys → likely different slots */

    /* Hash tag: {user}.following and {user}.followers → same slot */
    unsigned int ht1 = cluster_keyslot("{user}.following");
    unsigned int ht2 = cluster_keyslot("{user}.followers");
    ASSERT_EQ_INT((int)ht1, (int)ht2);

    /* Node ID should be 40 chars hex */
    ASSERT_EQ_INT((int)strlen(cs.node_id), NODE_ID_LEN);

    /* Cluster info should work */
    char buf[2048];
    cluster_info(&cs, buf, sizeof(buf));
    ASSERT_TRUE(strstr(buf, "cluster_enabled:1") != NULL);
    ASSERT_TRUE(strstr(buf, "cluster_slots_assigned:16384") != NULL);
}

static void test_pubsub_registry(void) {
    TEST_SUITE("Core / PubSub Registry");
    PubSubRegistry reg;
    pubsub_init(&reg);

    /* Publish to empty channel → 0 delivered */
    ASSERT_EQ_INT(pubsub_publish(&reg, "news", "hello"), 0);

    /* Unsubscribe from non-existent → no crash */
    pubsub_unsubscribe_all(&reg, 999);
...
void test_core(void) {
    ...
    test_zset();
    test_store_ttl();
    test_cluster_hashslot();
    test_pubsub_registry();
}
