...
#
#  Optional:
#    make ENABLE_LUA=1  → enable Lua scripting (needs liblua5.4-dev)
# ─────────────────────────────────────────────────────────
...
ifdef ENABLE_LUA
  CFLAGS  += -DENABLE_LUA $(shell pkg-config --cflags lua5.4 2>/dev/null)
  LDFLAGS += $(shell pkg-config --libs lua5.4 2>/dev/null || echo "-llua5.4 -lm")
endif
...
REPLICATION_OBJS = \
    $(SRC)/replication/replication.o

SCRIPTING_OBJS = \
    $(SRC)/scripting/scripting.o

COMMON_OBJS = $(CORE_OBJS) $(CMD_OBJS) $(ENGINE_OBJS) $(PERSIST_OBJS) \
              $(CONFIG_OBJS) $(PUBSUB_OBJS) $(CLUSTER_OBJS) \
              $(REPLICATION_OBJS) $(SCRIPTING_OBJS)
...
clean:
	...
	rm -f miniredis.rdb.tmp miniredis.aof.rewrite.tmp
