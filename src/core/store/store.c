...
/* INCR/DECR/INCRBY: atomically increment a string-stored integer.
   Returns 0 on success, -1 on wrong type, -2 if value is not an integer. */
int incrKey(Store *store, const char *key, long long amount, long long *result) {
    Object *obj = getKey(store, key);
    if (!obj) {
        /* Key doesn't exist — create with value 0, then add amount */
        *result = amount;
        char buf[32];
        snprintf(buf, sizeof(buf), "%lld", amount);
        setKey(store, key, createStringObj(buf));
        return 0;
    }
    if (obj->type != OBJ_STRING) return -1;  /* wrong type */
    StringObject *sobj = (StringObject*)obj->ptr;
    char *endptr;
    long long val = strtoll(sobj->value, &endptr, 10);
    if (*endptr != '\0') return -2;  /* not an integer */
    val += amount;
    *result = val;
    char buf[32];
    snprintf(buf, sizeof(buf), "%lld", val);
    setStringValue(sobj, buf);
    return 0;
}

/* APPEND: append suffix to an existing string key, or create if missing.
   Returns 0 on success (new_len set), -1 on wrong type. */
int appendKey(Store *store, const char *key, const char *suffix, int *new_len) {
    Object *obj = getKey(store, key);
    if (!obj) {
        /* Key doesn't exist — create with the suffix as value */
        setKey(store, key, createStringObj(suffix));
        *new_len = (int)strlen(suffix);
        return 0;
    }
    if (obj->type != OBJ_STRING) return -1;  /* wrong type */
    *new_len = appendStringValue((StringObject*)obj->ptr, suffix);
    return 0;
}
...
/* LRANGE with start/stop indices (supports negative indexing) */
char* lrangeKeyIdx(Store *store, const char *key, int start, int stop) {
    Object *obj = getKey(store, key);
    if (!obj) return NULL;
    if (obj->type != OBJ_LIST) return NULL;
...
int llenKey(Store *store, const char *key) {
    Object *obj = getKey(store, key);
    if (!obj) return 0;
    if (obj->type != OBJ_LIST) return -1;
    return llen((ListObject*)obj->ptr);
}
...
/* ------------------------------------------------------------------ */
/*  KEYS pattern matching (glob-style)                                  */
/* ------------------------------------------------------------------ */
        ...
        if (*pattern == '*') {
            ...
            while (*pattern && *pattern != ']') {
                if (pattern[1] == '-' && pattern[2] && pattern[2] != ']') {
                    if (*string >= pattern[0] && *string <= pattern[2]) matched = 1;
                    pattern += 3;
                } else {
                    if (*string == *pattern) matched = 1;
                    pattern++;
                }
            }
            if (*pattern == ']') pattern++;
...
char* keysPattern(Store *store, const char *pattern) {
    if (!store || !pattern) return NULL;
    StrBuf sb;
    sb_init(&sb);
    int found = 0;
    for (int i = 0; i < store->size; i++) {
        Entry *curr = store->buckets[i];
        while (curr) {
            /* Check for expired keys (lazy) */
            if (curr->expires_at != -1 && curr->expires_at <= time(NULL)) {
                curr = curr->next;
                continue;
            }
            if (glob_match(pattern, curr->key)) {
                sb_append(&sb, curr->key);
                sb_append(&sb, ",");
                found++;
            }
