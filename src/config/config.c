...
MiniRedisConfig config_defaults(void) {
    ...
    cfg.replicaof_host[0] = '\0';  /* empty = not a replica */
    ...
    *cfg = config_defaults();
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *space = '\0';
        ...
        } else if (strcmp(directive, "replicaof") == 0) {
            /* Format: replicaof <host> <port> */
            char *sp = strchr(value, ' ');
            if (!sp) {
                fprintf(stderr, "[config] line %d: replicaof requires 'host port'\n", lineno);
            } else {
                *sp = '\0';
                strncpy(cfg->replicaof_host, value, sizeof(cfg->replicaof_host) - 1);
                cfg->replicaof_host[sizeof(cfg->replicaof_host) - 1] = '\0';
                cfg->replicaof_port = atoi(sp + 1);
            }
        } else {
...
void config_print(const MiniRedisConfig *cfg) {
    ...
    if (cfg->replicaof_host[0])
        printf("  replicaof        : %s %d\n", cfg->replicaof_host, cfg->replicaof_port);
    else
        printf("  replicaof        : (none — running as master)\n");
    printf("================================\n");
}
